...
def main():
    ...
    encoder = get_resnet34(pretrained=False)
    ...
    Path(CFG["baseline_weights"]).parent.mkdir(parents=True, exist_ok=True)
    torch.save(model.state_dict(), CFG["baseline_weights"])
    logger.info("Saved baseline weights -> %s", CFG["baseline_weights"])
